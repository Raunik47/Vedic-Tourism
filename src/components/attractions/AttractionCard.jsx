...
export default function AttractionCard({ attraction }) {
  ...
  const isVideo =
    (typeof src === "string" && /\.(mp4|webm|ogg)(\?.*)?$/.test(src)) ||
        ...
        <div className="relative h-64 w-full bg-gray-100">
          {isVideo ? (
            <video
              ...
              autoPlay={false}
              ref={(el) => {
                if (el) hovered ? el.play() : el.pause();
              }}
            />
          ) : (
            <img
          ...
          <div className="absolute left-4 bottom-4 z-20">
            <div className="flex items-center gap-3">
              <div className="text-3xl bg-white/20 rounded-full p-2">
                {attraction.icon}
              </div>
        ...
        <div className="p-5 flex flex-col gap-4 flex-grow">
          <div className="flex items-start justify-between">
            <div>
              ...
              <h4 className="text-lg font-semibold text-gray-900 mt-1">
                {attraction.highlight || attraction.title}
              </h4>
            </div>
          ...
          <div className="flex flex-wrap justify-between items-center gap-3 mt-auto">
            <button
              ...
              className="flex-1 md:flex-none px-4 py-2 bg-white text-orange-600 rounded-lg font-semibold shadow hover:scale-105 transition text-sm md:text-base"
            >
              Learn more
            </button>
            <button className="flex-1 md:flex-none px-6 py-2 bg-gradient-to-r from-orange-500 to-orange-600 text-white rounded-lg font-semibold shadow hover:shadow-lg transition text-sm md:text-base">
              Book
            </button>
          </div>
        </div>
